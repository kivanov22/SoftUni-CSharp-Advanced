...
namespace Vehicles
{
    public class StartUp
    {
        ...
        private static Vehicle CreateVehicle()
        {
            string[] parts = Console.ReadLine().Split();
            string type = parts[0];
            double fuelQantity = double.Parse(parts[1]);
            ...
            if (type == nameof(Car))
            {
                vehicle = new Car(fuelQantity, fuelConsumption, tankCapacity);
            }
            else if (type == nameof(Truck))
            {
                vehicle = new Truck(fuelQantity, fuelConsumption, tankCapacity);
            }
            else if (type == nameof(Bus))
            {
                vehicle = new Bus(fuelQantity, fuelConsumption, tankCapacity);
            }
            return vehicle;
        }
    }
